...
def build_dashboard(conn: sqlite3.Connection, report: dict) -> None:
    ...
    channel_rows = query_rows(conn, """
        SELECT
            traffic_channel,
            COUNT(*) AS sessions,
            SUM(converted) AS converted,
            ROUND(SUM(converted) * 100.0 / COUNT(*), 1) AS conversion_rate
        FROM ml_session_conversion_features
    ...
    top_category_revenue = cat_rows[0]["revenue"] if cat_rows else 0
    total_category_revenue = sum((r["revenue"] or 0) for r in cat_rows)
    top_category_share = round(top_category_revenue / total_category_revenue * 100, 1) if total_category_revenue else 0
    top_channel = channel_rows[0]["traffic_channel"] if channel_rows else "N/A"
    top_channel_rate = channel_rows[0]["conversion_rate"] if channel_rows else 0
    ...
    channel_labels_js = json.dumps([r["traffic_channel"] for r in channel_rows])
    channel_rates_js  = json.dumps([r["conversion_rate"] for r in channel_rows])
    channel_sessions_js = json.dumps([r["sessions"] for r in channel_rows])
